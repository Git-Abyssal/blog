...
describe('admin app boundary', () => {
  ...
  it('opens article management from the admin root path', async () => {
    ...
    const activeNavigationLink = screen.getByRole('link', { name: '文章管理' })
    expect(activeNavigationLink).toHaveAttribute('aria-current', 'page')
    expect(activeNavigationLink).toHaveClass('text-brand-blue', 'dark:text-blue-300')
    expect(activeNavigationLink).not.toHaveClass('bg-blue-50', 'dark:bg-blue-500/10')
