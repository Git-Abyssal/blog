...
describe('Write', () => {
  ...
  it.each(['/write', '/write?edit=7'])(
    ...
    async (entry) => {
      ...
      expect(backLink.closest('.admin-surface')).toHaveClass('grid', 'grid-cols-[auto_1fr_auto]')
      expect(titleInput).toHaveClass('col-start-2', 'row-start-1')
      expect(backLink).toHaveClass('h-11', 'w-11', 'sm:w-auto')
      expect(backLink.querySelector('span')).toHaveClass('hidden', 'sm:inline')
      expect(saveButton).toHaveClass('h-11', 'w-11', 'sm:w-auto')
      expect(saveButton.querySelector('span')).toHaveClass('hidden', 'sm:inline')
      expect(publishButton).toHaveClass('h-11', 'w-11', 'sm:w-auto')
      expect(publishButton.querySelector('span')).toHaveClass('hidden', 'sm:inline')
  ...
  it('offers an edit and preview switch for narrow screens', async () => {
    ...
    expect(textarea).toHaveClass('rounded-2xl', 'md:rounded-bl-2xl', 'md:rounded-br-none', 'md:rounded-t-none')
    expect(textarea.parentElement?.parentElement).toHaveClass('rounded-2xl', 'md:rounded-b-2xl', 'md:rounded-t-none')
    ...
    expect(editButton).not.toHaveClass('border-b-2', 'border-brand-blue')
    fireEvent.click(previewButton)
    expect(previewButton).toHaveAttribute('aria-pressed', 'true')
    expect(previewButton).not.toHaveClass('border-b-2', 'border-brand-blue')
  ...
  it('uses consistent custom menus for categories and tags', async () => {
    ...
    expect(category.parentElement?.parentElement?.parentElement).toHaveClass('grid', 'grid-cols-2')
    expect(category.parentElement).toHaveClass('flex-1')
    expect(category).toHaveClass('w-full')
