...
describe('AdminArticles', () => {
  ...
  it('renders the article list as a flat divided section', async () => {
    ...
    expect(articleTitle.closest('li')).toHaveClass('flex-row', 'items-center', 'justify-between', 'gap-1.5', 'pl-3', 'pr-[0.1875rem]', 'py-2.5')
    expect(articleTitle.closest('li')).not.toHaveClass('flex-col')
    expect(articleTitle).toHaveClass('min-w-0', 'truncate', 'leading-6')
    expect(articleTitle).not.toHaveClass('flex-1')
    expect(articleTitle.parentElement).toHaveClass('flex', 'items-center', 'gap-2')
    expect(articleTitle.nextElementSibling).toHaveTextContent('已发布')
    expect(articleTitle.nextElementSibling).toHaveClass('shrink-0', 'whitespace-nowrap')
